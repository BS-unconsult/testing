...
Every cloud session runs in a fresh container: previous chat transcripts
don't exist on disk in a new one. The only durable cross-session storage is
this git repo, so session state must be checkpointed into it before a
session ends, or it's simply gone — and the next session needs to know to
look for it there.
...
1. Distill the current session into exactly these five sections, under 500
   ...
   Distill primarily from the in-context conversation; if earlier context
   was summarized away, supplement from the session's own transcript at
   `/root/.claude/projects/-home-user/<current-session-id>.jsonl` (the
   newest `.jsonl` file there).
...
4. Update `.claude/handovers/index.md`: prepend one line under the heading,
   newest first:
   ```
   # Checkpoint index (newest first)
   - YYYY-MM-DD — <short topic> — <dated-filename>.md — <branch>
   ```
   If `index.md` doesn't exist yet, create it with the heading first.

5. Commit the dated file, `latest.md`, and `index.md` together with message
   `checkpoint: <topic> (YYYY-MM-DD)`, and push with
...
6. Confirm to the user in one line that the checkpoint is saved, and that
...
2. From that same ref, also read `.claude/handovers/index.md` to build a
   list of recent topics. If `index.md` is missing, reconstruct it
   best-effort from `git log --all --name-status -- .claude/handovers/`,
   which surfaces dated files across all refs.

3. If a checkpoint is found: brief the user with a compressed "Here's where
   we left off" summary of the NEWEST checkpoint's five sections, ending
   with "Start here". Then, if the index has other distinct topics (same
   topic collapsed to its latest date), add one line: "There are also
   checkpoints for *X* (YYYY-MM-DD) and *Y* (YYYY-MM-DD); say the topic to
   resume one of those instead." — up to 4, omitted if none. Don't ask
   clarifying questions before this briefing.

4. If the user names a topic — in their initial request ("resume the X
   work") or after the briefing — look it up in the index for its most
   recent dated file and brief from THAT file instead of `latest.md`.
   Resuming an older topic must not modify `latest.md` or `index.md`; only a
   new checkpoint writes to them.

5. Fallback if NO checkpoint exists anywhere: reconstruct context from git
...
6. Never delete old dated handover files. They are the permanent archive.
